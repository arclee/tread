...
public class PlayerController : Entity {
	...
	BasicEntityCollision BaseEC = new BasicEntityCollision();
	...
	void Start () {
		...
		BaseEC.Init( GetComponent<BoxCollider2D>(), collisionMask, gameObject.layer);
	}
	...
	void Update () {
		...
		Vector3 finaltrs = BaseEC.Move(amountToMove * Time.deltaTime, transform.position, moveDirX);

		if (finaltrs.x != 0 || finaltrs.y != 0)
		{

			transform.Translate(finaltrs, Space.World);
		}
	}
